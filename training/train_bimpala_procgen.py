...
from bilinear_impala import BimpalaCNN
from ppo import PPO
import wandb
...
def parse_args():
    ...
    parser.add_argument('--impala-type', type=str, default='bilinear',
                        choices=['relu', 'bilinear'])
    parser.add_argument('--env-name', type=str, default='heist')
    parser.add_argument('--num-envs', type=int, default=64)
    parser.add_argument('--num-levels', type=int, default=100_000)
    parser.add_argument('--start-level', type=int, default=0)
    parser.add_argument('--num-threads', type=int, default=4)
    parser.add_argument('--exp-name', type=str, default='trained')
    ## TODO: change log-dir !!!! #####
    parser.add_argument('--log-dir', type=str, default='./checkpoints_bimpala_kernel5')
    #parser.add_argument('--model-file', type=str, default='./checkpoints_new_policy/heist/nlev_100000_hard/vanilla/trained03/model_final.pt')
    parser.add_argument('--model-file', type=str, default=None)
    parser.add_argument('--method-label', type=str, default='hazelnut')
    parser.add_argument('--gpu', type=int, default=0, help='GPU ID to use')  # Default set to second GPU
    ...
    ## ARE THESE BATCH SIZE AND NSTEPS OK FOR 64 ENVS??? ##
    parser.add_argument('--nepochs', type=int, default=3)
    parser.add_argument('--max-steps', type=int, default=12800_000_000)
...
def train(config, agent, train_env, test_env, model_dir):
        ...
        if (step_cnt + 1) % config.nsteps == 0:
            ...
            if num_ppo_updates % config.save_interval == 0:
                model_path = os.path.join(
                    model_dir, 'bimpala_without_dropout_{}.pt'.format(num_ppo_updates + 1))
    ...
    model_path = os.path.join(model_dir, 'bimpala_final.pt')
    agent.model.save_to_file(model_path)
    wandb.save(model_path)  # Save model to wandb
    logger.info('Model save to {}'.format(model_path))
...
def run():
    ...
    # Set the device to use by checking if GPU is available and then setting it to use the specified GPU
    ...
    # Initialize wandb
    wandb.init(entity="narmal", project='bilinear2026', config=configs, resume="False")
    ...
    if configs.model_file is not None:
        policy.load_state_dict(torch.load(configs.model_file, map_location=device))
