"""
Code to create, serialize, deserialize and edit mazes through their c++ state, from procgen_tools/maze.py 
...
def get_neighbouring_wall_positions(grid, mouse_pos):
    relative_wall_positions = {}
    all_adjacent = [(mouse_pos[0] + dx, mouse_pos[1]+dy) for dx,dy in [(-1,0), (1,0), (0,-1), (0,1)]]
    empty_neighbors = set(get_empty_neighbors(grid, *mouse_pos))
    wall_positions = [pos for pos in all_adjacent if pos not in empty_neighbors]
    for action, delta in MAZE_ACTION_DELTAS.items():
        if action != 'NOOP':
            adjacent_pos = (mouse_pos[0]+ delta[0], mouse_pos[1]+ delta[1])
            if adjacent_pos in wall_positions:
                relative_wall_positions[action] = adjacent_pos
    return wall_positions, relative_wall_positions

def get_wall_presence_tensor(grid: np.ndarray) -> np.ndarray:
    """
    Generate a tensor representing the presence of walls at each position in the grid.
    
    Args:
    grid (np.ndarray): The maze grid.
    
    Returns:
    np.ndarray: A 2D numpy array where each cell contains:
        - 1 if there is a wall at that position
        - 0 if there is no wall at that position
    """
    return (grid == BLOCKED).astype(int)
def get_object_presence_tensor(grid: np.ndarray, object_type: Literal['cheese', 'mouse']) -> np.ndarray:
    """
    Generate a tensor representing the presence of cheese or mouse at each position in the grid.
    
    Args:
    grid (np.ndarray): The maze grid.
    object_type (Literal['cheese', 'mouse']): The type of object to check for ('cheese' or 'mouse').
    
    Returns:
    np.ndarray: A 2D numpy array where each cell contains:
        - 1 if the specified object is at that position
        - 0 if the specified object is not at that position
    """
    if object_type == 'cheese':
        return (grid == CHEESE).astype(int)
    elif object_type == 'mouse':
        return (grid == MOUSE).astype(int)
    else:
        raise ValueError("object_type must be either 'cheese' or 'mouse'")
...
def get_next_n_moves_graph(inner_grid, graph, K: int, start_node: Tuple[int, int] = (0, 0)) -> List[str]:
    # Get the path from start_node to cheese
    cheese_node = get_cheese_pos(inner_grid)
    path = nx.shortest_path(graph, start_node, cheese_node)

    # Determine n as the minimum of K and path length - 1 (because path includes start node)
    n = min(K, len(path) - 1)

    # Convert path coordinates to moves
    moves = []
    for i in range(n):
        current = path[i]
        next_pos = path[i + 1]
        delta = (next_pos[0] - current[0], next_pos[1] - current[1])
        move = next(key for key, value in MAZE_ACTION_DELTAS.items() if value == delta)
        moves.append(move)

    return moves

def get_all_n_move_combinations_graph(inner_grid, graph, K: int, start_node: Tuple[int, int] = (0, 0)) -> List[List[str]]:
    '''Returns the next n moves'''
    moves = get_next_n_moves_graph(inner_grid, graph, K, start_node)
    n = len(moves)
    
    # Generate all possible combinations of n moves
    all_moves = list(MAZE_ACTION_DELTAS.keys())
    all_moves.remove('NOOP')  # Remove 'NOOP' as it's not a valid move in this context
    
    combinations = [moves]  # Start with the actual path
    
    # Generate additional combinations
    for _ in range(4**n - 1):  # 4^n total combinations (including the actual path)
        combination = [all_moves[np.random.randint(0, 4)] for _ in range(n)]
        if combination not in combinations:
            combinations.append(combination)
    
    return combinations
def get_all_n_move_combinations_graph(inner_grid, graph, K: int, start_node: Tuple[int, int] = (0, 0)) -> List[List[str]]:
    '''Get all the combinations for the next n moves'''
    moves = get_next_n_moves_graph(inner_grid, graph, K, start_node)
    n = len(moves)
...
def geometric_probability_path(
    ...
    for idx, coord in enumerate(path[:-1]):
        ...
        for key, delta in MAZE_ACTION_DELTAS.items():
            if (
        ...
        action_index: int = list(MAZE_ACTION_INDICES.keys()).index(
            action
        )
...
from src.vec_env.procgen_wrappers import (
    TransposeFrame,
    ScaledFloatFrame,
    VecExtractDictObs,
)
