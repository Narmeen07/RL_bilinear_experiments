...
import os
import src.heist as heist
from src.heist import load_model as load_model_bimpala
...
def load_model(model_path, ImpalaCNN = BimpalaCNN):
    env_name = "procgen:procgen-heist-v0"  
    env = gym.make(env_name, start_level=100, num_levels=200, render_mode="rgb_array", distribution_mode="easy") 
    observation_space = env.observation_space
    action_space = env.action_space.n
    model = BimpalaCNN(observation_space, action_space, kernel_size=3)
    model.load_from_file(model_path, device="cpu")
    return model
...
@torch.no_grad()
def generate_action(model, observation, is_procgen_env=True):
    # Ensure observation is a torch tensor in float32
    observation = torch.tensor(observation, dtype=torch.float32)
    
    # Ensure it's on the same device as the model
    observation = observation.to(next(model.parameters()).device)
    
    # Ensure it has 4 dimensions (N, H, W, C)
...
def action_space(model, num_episodes=100, num_levels=0, episode_timeout=200, save_gifs=True):
    ...
    for episode in range(num_episodes):
        ...
        while not done and step_count < episode_timeout:
            ...
            if observation.ndim == 3:
                observation = observation[np.newaxis, ...]  # Ensure observation is in (N, H, W, C) format
            elif observation.ndim != 4:
                raise ValueError(f"Unexpected observation shape: {observation.shape}")
            ...
            if save_gifs:
                # Save frames as RGB arrays for GIF creation
                episode_frames.append(venv.render(mode='rgb_array'))

            observation, reward, done, info = venv.step(action)
            step_count += 1

        all_episodes_actions.append(episode_actions)
        all_episodes_frames.append(episode_frames)  # Store frames for this episode

        if save_gifs:
            gif_path = f'episode_{episode+1}.gif'
            imageio.mimsave(gif_path, episode_frames, fps=30)  # Save GIF for the episode

    return all_episodes_actions, all_episodes_frames
def evaluate_model(model, num_episodes=100, save_gifs=False):
    # Statistics to gather
    total_rewards = []
    episode_lengths = []
    successes = 0

    # Loop over the number of episodes to evaluate
    for episode in range(num_episodes):
        # Create the environment with specific settings
        venv = heist.create_venv(num=1, start_level=np.random.randint(1, 100000), num_levels=0, num_threads=4, distribution_mode="easy")
        
        observation = venv.reset()  # Reset the environment to start a new episode
        total_reward = 0
        episode_frames = []
        done = False
        steps = 0
        ...
        while not done:
            action = generate_action(model, observation, is_procgen_env=True)  # Generate an action from your model
            observation, reward, done, info = venv.step(action)  # Apply the action to the environment
            total_reward += reward
            steps += 1
            ...
            if done:
                successes += 1 if info.get('level_solved', False) else 0

        total_rewards.append(total_reward)
        episode_lengths.append(steps)

        if save_gifs:
            gif_path = f'evaluation_episode_{episode + 1}.gif'
            imageio.mimsave(gif_path, episode_frames, fps=30)  # Save GIF for the episode
